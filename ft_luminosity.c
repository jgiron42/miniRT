...
/*   Updated: 2020/02/08 17:04:40 by jgiron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_coord	ft_normal_vector (t_mrt_list object, t_coord point, t_all_info all_info)
{
	...
	length = sqrt((ft_dist(object.pos, point) * ft_dist(object.pos, point)) - (object.radius * object.radius));
...
//	if(fabs(ft_dist(ft_vect_product(object.vect, ft_vect_calc(object.pos, all_info.cameras->pos)), (t_coord){})) >
//		object.length * object.radius)
	if (ft_scalar(result, ft_vect_calc(all_info.cameras->pos, point)) < 0)
		return (result);
	else
		return (ft_vect_calc(result, (t_coord){}));
}

void	ft_luminosity2(t_color *color, t_all_info all_info, t_mrt_list object, t_coord point)
{
	double factor;
	t_coord	normal_vect;
	t_coord light_vect;
	...
	{
		*color = (t_color){
			.r = color->r + ((double)object.color.r / 255) * all_info.lights->color.r * factor * all_info.lights->light,
			.g = color->g + ((double)object.color.g / 255) * all_info.lights->color.g * factor * all_info.lights->light,
			.b = color->b + ((double)object.color.b / 255) * all_info.lights->color.b * factor * all_info.lights->light
		};
	}
}

t_color	ft_luminosity(t_all_info all_info, t_mrt_list current, t_coord original_vector)
{
	t_mrt_list	*objects;
	t_coord		point;
	t_coord		tmp;
	...
	result = (t_color){
			(int)((double)current.color.r * (((double)all_info.scene_info.color.r / 255) * (double)all_info.scene_info.light)),
			(int)((double)current.color.g * (((double)all_info.scene_info.color.g / 255) * (double)all_info.scene_info.light)),
			(int)((double)current.color.b * (((double)all_info.scene_info.color.b / 255) * (double)all_info.scene_info.light)),
	};
	while (all_info.lights)
	{
		...
		while (objects)
		{
			tmp = ft_intersection(ft_vect_calc(point, all_info.lights->pos), point, *objects);
			if (!isnan(tmp.x) && ft_dist(tmp, point) < ft_dist(all_info.lights->pos, point) - PRECISION)
			{
				is_lighted = 0;
				break;
			}
