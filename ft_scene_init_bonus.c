...
/*   Updated: 2020/02/16 19:24:19 by jgiron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_r_init(t_info *scene_info, char *line)
{
	...
	if (*line == 0 || scene_info->dim.i < 1 || scene_info->dim.j < 1)
		return (-1);
...
static int	ft_c_init(t_all_info *all_info, char *line)
{
	...
	{
		line += 2;
		all_info->scene_info.stereo = ft_atof(&line);
		all_info->scene_info.stereo_depth = ft_atof(&line);
		return (0);
	}
...
int			ft_scene_init(t_all_info *all_info, char *line)
{
	...
	else if (*line == 'A' || ft_strstart(line, "aa"))
		return (ft_a_init(&all_info->scene_info, line));
	else if (*line == 'c')
		return (ft_strstart(line, "st") || ft_c_init(all_info, line));
	else if (ft_strstart(line, "sk"))
	{
		...
		if (ft_strnstr(line, ".xpm", ft_strlen(line)))
			all_info->scene_info.skysphere.img_ptr = mlx_xpm_file_to_image(
all_info->mlx_ptr, line, &all_info->scene_info.skysphere.dim.i,
&all_info->scene_info.skysphere.dim.j);
		else if (ft_strnstr(line, ".png", ft_strlen(line)))
			all_info->scene_info.skysphere.img_ptr = mlx_png_file_to_image(
all_info->mlx_ptr, line, &all_info->scene_info.skysphere.dim.i,
&all_info->scene_info.skysphere.dim.j);
