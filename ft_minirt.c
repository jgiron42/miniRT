...
/*   Updated: 2020/02/13 21:57:11 by jgiron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_img_address(void *img_ptr, int bits_per_pixel, int size_line,
		int endian)
{
...
t_coord	original_vector(t_all_info all_info, t_pix pixel)
{
	...
	x_vect = all_info.cameras->vect.x == 0 && all_info.cameras->vect.y == 0 ? ft_coord(0,1,0) : ft_unit_vect(ft_vector_rotation(ft_coord(all_info.cameras->vect.x, all_info.cameras->vect.y, 0), ft_coord(0,0,1), M_PI_2));
	y_vect = all_info.cameras->vect.x == 0 && all_info.cameras->vect.y == 0 ? ft_coord(1,0,0) : ft_unit_vect(ft_vector_rotation(x_vect, all_info.cameras->vect, M_PI_2));
	factor = (((double)pixel.i * 2) - (double)all_info.scene_info.dim.i) / all_info.scene_info.dim.i;
	x_vect = (t_coord){
		x_vect.x * factor,
		x_vect.y * factor,
		x_vect.z * factor
	};
	factor = (-((double)pixel.j * 2) + (double)all_info.scene_info.dim.j) / all_info.scene_info.dim.j;
	y_vect = (t_coord){
			y_vect.x * factor,
			y_vect.y * factor,
			y_vect.z * factor
	...
	dst = ft_coord_addition(x_vect, y_vect);
	dst = ft_coord_addition(dst, fov_vector);
	return (ft_unit_vect(dst));
}
