...
/*   Updated: 2020/02/13 16:33:02 by jgiron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color ft_skysphere(t_all_info all_info, t_coord vect)
{
	...
	if ((vect.x == 0 && vect.y == 0) || !all_info.scene_info.skysphere.img_ptr)
		return ((t_color){});
	vect_hor = ft_unit_vect(ft_coord(vect.x, vect.y, 0));
	pixel.i = (int)(((M_PI + ((asin(vect.x) / fabs(asin(vect.x))) * acos(vect_hor.y)))/ (2*M_PI)) * (double)all_info.scene_info.skysphere.dim.i);
	pixel.j = (int)((acos(vect.z) / M_PI) * (double)all_info.scene_info.skysphere.dim.j);
	address = ft_get_img_address(all_info.scene_info.skysphere.img_ptr,
			24, all_info.scene_info.skysphere.dim.i, 1);
	if (pixel.i >= all_info.scene_info.skysphere.dim.i || pixel.j >= all_info.scene_info.skysphere.dim.j)
		return ((t_color){});
	return ((t_color){
